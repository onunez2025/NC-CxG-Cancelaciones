...
export function MainLayout({ children }: MainLayoutProps) {
    ...
    const { user, hasPermission, logout, sessionConfig } = useAuth();
    const appConfig = useAppConfig();
    const logoUrl = appConfig?.logoUrl || '/Logo.png';

    const COLLAPSED_KEY = 'cxg_sidebar_collapsed';
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-3 lg:gap-4 group cursor-default">
                            <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-[1.25rem] bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-white/10 shadow-lg shadow-slate-200/40 dark:shadow-none flex items-center justify-center group-hover:scale-110 transition-all duration-500">
                                <img src={logoUrl} alt="Logo" className="w-6 h-6 lg:w-7 lg:h-7 object-contain" />
                            </div>
                            <div className="flex flex-col">
                                <span className="font-black text-xs lg:text-sm tracking-tight text-foreground uppercase pt-1">Mesa de Atención</span>
                                <div className="flex items-center gap-2">
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                    <span className="text-[9px] lg:text-[10px] font-black text-muted-foreground tracking-widest uppercase opacity-60 hidden sm:inline">Gestión y Seguimiento de Servicios</span>
                                    <span className="text-[9px] lg:text-[10px] font-black text-muted-foreground tracking-widest uppercase opacity-60 inline sm:hidden">Mesa</span>
                                </div>
                            </div>
                        </div>
                    </div>
